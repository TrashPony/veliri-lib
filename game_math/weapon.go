...
func GetWeaponFirePositions(ownerX, ownerY, ownerScale int, ownerRotate float64, gunRotate float64, weaponXAttach, weaponYAttach int,
	...
	realPos := make([]*Positions, len(weaponFirePositions), len(weaponFirePositions))
	...
	for i, pos := range weaponFirePositions {
		...
		realPos[i] = &Positions{X: int(newX), Y: int(newY), Radius: int(float64(pos.Radius) * sizeOffset), Collision: pos.Collision}
	}
...
func GetWeaponFirePosition(ownerX, ownerY, ownerScale int, ownerRotate float64, gunRotate float64, weaponXAttach, weaponYAttach int,
	weaponFirePositions []*coordinate.Coordinate, slotXAttach, slotYAttach float64, position int) *Positions {
	...
	// прибавляем точку обьекта к точке оружия
	return &Positions{X: int(newX), Y: int(newY), Radius: int(float64(pos.Radius) * sizeOffset), Collision: pos.Collision}
}
